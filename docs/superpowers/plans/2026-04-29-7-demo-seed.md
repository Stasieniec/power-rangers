...
export const TEAMS = [
  {
    ...
    name: "Convex Lab",
    description:
      "A small applied-ML team focused on health-economics modeling.",
...
The seed runs locally but talks to remote D1 + KV via `wrangler d1 execute` and `wrangler kv key put`, OR — simpler — runs *on a Worker* triggered by a one-off HTTP route that's dev-only. We'll use the latter for cleaner Drizzle access.
...
export async function runSeed() {
  ...
  for (const teamId of ["team_lattice", "team_helix"]) {
    ...
    for (const uid of userIds) {
      ...
      teamConcepts.push(
        ...cs.map((c) => ({ label: c.concept, weight: c.score }))
      );
    }
    const qs = await db
      .select()
    ...
    const finalScore = Math.max(
      0,
      Math.min(100, matchResult.baseScore + ai.adjustment)
    );
  ...
  for (const r of REPORTS) {
    ...
    const out = await translateReport({
      endGoal:
        PROJECTS.find((p) => p.id === r.projectId)?.endGoal ?? "",
...
Visit `/onboard?demo=fallback` and pick a demo researcher. Profile builds instantly using the canned response (skipping OpenAlex *and* Gemini). Then visit `/onboard` (no query) and verify live mode still works.
